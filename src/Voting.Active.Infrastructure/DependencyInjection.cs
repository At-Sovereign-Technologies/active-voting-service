...
public static class DependencyInjection
{
        ...
        IConfiguration configuration)
    {
        ...
        var electoralServerSettings = configuration
            .GetSection("ElectoralServer")
            .Get<ElectoralServerSettings>()!;

        services.AddHttpClient<ElectoralServerClient>(client =>
        {
            client.BaseAddress = new Uri(electoralServerSettings.BaseUrl);

            client.DefaultRequestHeaders.Authorization =
                new AuthenticationHeaderValue("Bearer", electoralServerSettings.NodeSecret);
        });
